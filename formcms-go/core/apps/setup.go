...
func LoadAppsConfig() ([]string, error) {
	data, err := os.ReadFile("apps/apps.json")
	if err != nil {
...
func SetupApp(ctx context.Context, appName string, schemaService *services.SchemaService, dao relationdbdao.IPrimaryDao) error {
	schemasDir := filepath.Join("apps", appName, "schemas")
...
func SetupAppTestData(ctx context.Context, appName string, entityService services.IEntityService) error {
	filePath := filepath.Join("apps", appName, "data", "test_data.json")
